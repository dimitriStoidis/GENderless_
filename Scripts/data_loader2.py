"""
****************************************
Implementation by Dimitrios Stoidis

GENderless Extension
* Chanded the __gettitem__()
...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path):
        ...
        if spect[1].size < 2000:
            spect = np.pad(spect, ((0, 0), (0, 2000 - spect[1].size)), 'constant')

        else:
            spect = spect[:, 0:2000]  # take slice until 2000 for bigger utterances
...
def get_gender(identity, sample):
        gen_label = 0
        for j in range(len(identity)):
            if sample[0].split('/')[-1].split('-')[0] == identity[j]:
                if SEX[j] == 'F':
                    gen_label = 1
                elif SEX[j] == 'M':
                    # gen_label.append(0)
...
def load_subject_data():
    ...
    for line in file:
        ...
        if i > 12:
            fields = line.split("|")
            if fields[2] == ' train-clean-100  ':
                ID = np.append(ID, fields[0].strip())
                SEX = np.append(SEX, fields[1].strip())
                subset = np.append(subset, fields[2])
                minutes = np.append(minutes, fields[3])
                name = np.append(name, fields[4])

            if fields[2].strip() == 'test-clean' or fields[2].strip() == 'dev-clean':
                ID = np.append(ID, fields[0].strip())
                SEX = np.append(SEX, fields[1].strip())
                subset = np.append(subset, fields[2])
                minutes = np.append(minutes, fields[3])
...
ID, SEX, subset, minutes, name = load_subject_data()